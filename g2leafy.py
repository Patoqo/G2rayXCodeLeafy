...
def port_pids(port):
    ...
    try:
        out = subprocess.check_output(
            ["bash", "-c", f"ss -tlnpH 'sport = :{port}' 2>/dev/null"],
            text=True)
    ...
    if not pids:
        try:
            out = subprocess.check_output(["bash", "-c", f"fuser {port}/tcp 2>/dev/null"],
                                          text=True)
...
def port_in_use(port):
    try:
        out = subprocess.check_output(
            ["bash", "-c", f"ss -tlnH 'sport = :{port}' 2>/dev/null"], text=True)
        return bool(out.strip())
    except Exception:
        return False


def free_port(port, keep_pid=None):
...
def background_worker():
    ...
    while engine_running:
        ...
        if tick % 30 == 0:
            state["ports_ok"] = port_in_use(XRAY_PORT) and state["is_xray_running"]

        if tick % 60 == 0:
            try:
                subprocess.run(["gh", "codespace", "ports", "visibility",
                                f"{XRAY_PORT}:public", "-c", CODESPACE_NAME], capture_output=True)
                subprocess.run(["gh", "codespace", "ports", "visibility",
                                f"{SUB_PORT}:public", "-c", CODESPACE_NAME], capture_output=True)
            except Exception:
                pass
        time.sleep(1)
...
def generate_link(ip, tag):
    ...
    return (f"vless://{uid}@{ip}:{XRAY_PORT}?encryption=none&security=tls"
...
def draw_box(scr, y, x, h, w, title="", color=4):
    ...
    scr.attroff(color)
    if title:
        safe_add(scr, y, x + 2, f" {title} ", color | curses.A_BOLD)
...
def draw_card(scr, y, x, w, title, val, unit, icon, icon_color):
    ...
    safe_add(scr, y + 2, x + 2, str(val), c_color(1) | curses.A_BOLD)
    safe_add(scr, y + 2, x + 2 + len(str(val)) + 1, unit, c_color(4))
...
def keyhints_for_tab(tab):
    controls = {"Nav": [("\u2191\u2193", "tab"), ("q", "quit")]}
    if tab == 1:
        controls["Power"] = [("s", "start"), ("x", "stop"), ("r", "restart")]
    elif tab == 2:
        controls["Settings"] = [("1", "cap"), ("2", "upd"), ("3", "wake"), ("8", "reset")]
    elif tab == 3:
...
def popup_ask(scr, prompt, default=""):
    h, w = scr.getmaxyx()
    bw = min(66, max(42, w - 4))
    win = curses.newwin(7, bw, max(1, (h - 7) // 2), max(1, (w - bw) // 2))
    win.keypad(True)
    win.bkgd(' ', c_color(1))
    buf = str(default)
    curses.curs_set(1)
    ...
    while True:
        win.erase()
        win.box()
        win.addstr(0, 2, " Input ", c_color(3) | curses.A_BOLD)
        win.addstr(2, 2, prompt[:bw - 4], c_color(1))
        win.addstr(4, 2, "Enter = save    Esc = cancel", c_color(4))
        win.addstr(3, 2, "> ", c_color(3) | curses.A_BOLD)
        win.addstr(3, 4, buf[-(bw - 6):], c_color(1) | curses.A_BOLD)
        win.refresh()
        ch = win.getch()
        if ch == 27:
            res = None
...
def popup_msg(scr, title, msg):
    ...
    win.keypad(True)
    win.box()
    win.addstr(0, 2, f" {title} ", c_color(3) | curses.A_BOLD)
    win.addstr(3, 2, msg[:bw - 4], c_color(1))
    win.addstr(5, 2, "Press any key...", c_color(4))
    win.refresh()
    win.getch()
    scr.clear()
...
def popup_confirm(scr, title, msg):
    ...
    win.addstr(0, 2, f" {title} ", c_color(3) | curses.A_BOLD)
    win.addstr(3, 2, msg[:bw - 4], c_color(1))
    win.addstr(5, 2, "[y] yes      [n] no", c_color(4))
    win.refresh()
    while True:
        ch = win.getch()
...
def screen_dashboard(scr, h, w, mx, mw):
    ...
    for i, c in enumerate(cards):
        ww = cw + (1 if i < rem else 0)
        draw_card(scr, 3, cc, ww, c[0], c[1], c[2], c[3], c[4])
        cc += ww + gap
    ...
    pw2 = mw - pw1 - gap
    lc, rc = mx, mx + pw1 + gap
    draw_box(scr, pr, lc, ph, pw1, "Health & Quota", c_color(2))
    draw_box(scr, pr, rc, ph, pw2, "Network", c_color(3))
    ...
    render_spaced_kvs(scr, pr + 1, lc + 2, pw1 - 4, ph - 2, health_items)
    ...
    render_spaced_kvs(scr, pr + 1, rc + 2, pw2 - 4, ph - 2, net_items)

    chh = max(7, h - (pr + ph + 1))
    draw_dot_chart(scr, pr + ph + 1, mx, chh, mw, "Traffic Flow",
...
def screen_settings(scr, h, w, mx, mw):
    ...
    draw_box(scr, pr, lc, ph, pw1, "Identity & Quota", c_color(2))
    ...
    id_items = [
        ...
        ("1 \u00b7 Data Cap", d_s, c_color(2)),
        ("Time Left", fmt_hms(left), c_color(3))
    ]
    render_spaced_kvs(scr, pr + 1, lc + 2, pw1 - 4, ph - 2, id_items)

    draw_box(scr, pr, rc, ph, pw2, "Connection", c_color(3))
    wl_status = "OFF"
    wl_color = c_color(4)
    if wake_lock_active:
        wl_color = c_color(2)
    ...
    conn_items = [
        ...
        ("2 \u00b7 Sub Update", f"{settings.get('sub_update_mins', 1)} mins", c_color(1)),
        ("3 \u00b7 Wake Lock", wl_status, wl_color)
    ]
    render_spaced_kvs(scr, pr + 1, rc + 2, pw2 - 4, ph - 2, conn_items)
    ...
    draw_dot_chart(scr, hr, mx, hh, chw, f"CPU  {state['cpu_pct']:.0f}%",
                   ...
                   value_fmt=lambda v: f"{v:.0f}%")
    draw_dot_chart(scr, hr, mx + chw + 2, hh, chw2, f"RAM  {state['mem_pct']:.0f}%",
...
def screen_configgen(scr, h, w, mx, mw):
    ...
    for i in range(start, min(len(nodes), start + maxrows)):
        ...
        if i == cfg_sel:
            safe_add(scr, row, rc + 1, ("\u258c " + line).ljust(pw2 - 2), c_color(6) | curses.A_BOLD)
        else:
            safe_add(scr, row, rc + 3, line, c_color(1))
        row += 1
    if not nodes:
...
def screen_logs(scr, h, w, mx, mw):
    draw_box(scr, 3, mx, h - 4, mw, "xray.log", c_color(2))
    if os.path.exists(XRAY_LOG):
        try:
            with open(XRAY_LOG) as f:
                lines = f.readlines()[-(h - 8):]
            if not lines:
                safe_add(scr, 5, mx + 2, "(log is empty \u2014 engine running cleanly)", c_color(4))
...
def main(scr):
    ...
    while True:
        h, w = scr.getmaxyx()
        h, w = max(22, h), max(74, w)
        sbw = 26 if w >= 96 else (22 if w >= 80 else 0)
        mx = sbw + 2 if sbw > 0 else 2
        mw = w - mx - 1
        ...
        if nav_current == 1:
            screen_dashboard(scr, h, w, mx, mw)
        elif nav_current == 2:
            screen_settings(scr, h, w, mx, mw)
        elif nav_current == 3:
            screen_configgen(scr, h, w, mx, mw)
        elif nav_current == 4:
            screen_logs(scr, h, w, mx, mw)
        ...
        if nav_current == 2:
            ...
            if k in (ord('a'), ord('A')):
                ...
                build_subscription()
                popup_msg(scr, "Done", "Subscription rebuilt in raw format with your configs.")
